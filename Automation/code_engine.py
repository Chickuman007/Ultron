...
def _get_editor_window():
    editor_keywords = ["cursor", "visual studio", "code", "pycharm", "sublime", "notepad"]

    try:
        active_window = pyautogui.getActiveWindow()
        if active_window and any(
            keyword in active_window.title.lower() for keyword in editor_keywords
        ):
    ...
    try:
        for win in pyautogui.getAllWindows():
            ...
            if title and any(keyword in title for keyword in editor_keywords):
...
def code_fix():
    ...
    try:
        target_window = _get_editor_window()

        if target_window is None:
            print("❌ No editor window found to paste code")
            speak("Sir, I could not find your editor window. Please focus it once and try again.")
            return
        ...
        # Re-focus editor in case a local service/port prompt stole focus.
        target_window.activate()
        time.sleep(0.4)

        # Paste at current cursor location in the editor.
